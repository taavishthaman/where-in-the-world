...
import { setDark } from '../CountriesList/countriesSlice';

export default function Header() {
    
    const dispatch = useDispatch();
    const {isDark} = useSelector(state => state.countries);
    return <div className={`${styles.header} ${isDark ? styles.headerDark : ''}`}>
        <div className={`${styles.title} ${isDark ? styles.titleDark : ''}`}>
            Where in the world?
        </div>
        <div className={`${styles.modeSwitcher} ${isDark ? styles.modeSwitcherDark : ''}`} onClick={() => {
            dispatch(setDark());
        }}>
            {
